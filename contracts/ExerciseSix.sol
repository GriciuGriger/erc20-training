...
contract ExerciseSix is ExerciseFive {
    ...
    function withdraw(uint256 weiToWithdraw) payable external onlyOwner 
    {
        uint256 contractBalance = uint256(address(this).balance);
        uint256 costOfLeftTokens = _totalSupply * _sellPrice;

        require(contractBalance - weiToWithdraw > costOfLeftTokens, "The amount of ether to withdraw exceeds cost of tokens left");

        (bool success, ) = payable(msg.sender).call{value: weiToWithdraw}("");   
        require(success, "Failed to withdraw Ether");     

        emit ETHWithdrawn(msg.sender, weiToWithdraw);
    }
